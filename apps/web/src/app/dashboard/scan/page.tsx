  ...
  async function handleStamp() {
    ...
    try {
      const res = await fetch(`${API_URL}/api/v1/customers/${customer.id}/stamp`, {
        ...
        body: JSON.stringify({ program_id: programId, source: "QR_SCAN" }),
      });
      ...
      <div className="rounded-2xl border border-gray-200 bg-white p-4 sm:p-6 space-y-3">
        ...
        <div className="flex flex-col sm:flex-row gap-2">
          <input
            ...
            className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-slate-500 focus:border-slate-500"
          />
          <button
            onClick={findByEmail}
            disabled={emailLoading}
            className="inline-flex items-center justify-center rounded-lg bg-emerald-500 text-white px-4 py-2 text-sm font-medium hover:bg-emerald-600 disabled:opacity-50"
          >
