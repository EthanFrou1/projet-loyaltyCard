 ...
 *
 * Affiche :
 *   - Confirmation de création de la carte
...
export default function WelcomePage() {
  ...
  async function handleGoogleWallet() {
    ...
    try {
      const token = localStorage.getItem("access_token"); // null pour les clients non connectés
      ...
      // Silencieux — on affiche juste un message de fallback
    } finally {
    ...
    <div className="min-h-screen bg-gradient-to-b from-blue-50 to-white flex flex-col items-center justify-center p-4">
      <div className="w-full max-w-sm space-y-6 text-center">
        ...
        <div className="space-y-3">
          {/* Apple Wallet */}
          <a
            href={`${API_URL}/api/v1/wallet/apple/${id}/download`}
            className="flex items-center justify-center gap-3 w-full py-3.5 px-4 bg-black text-white font-semibold rounded-xl hover:bg-gray-900 transition-colors"
          >
            <AppleIcon />
        ...
        <p className="text-xs text-gray-400 px-4">
          Votre carte s'affiche dans votre application Wallet.
          Le commerçant scannera votre QR code à chaque visite pour ajouter un tampon.
        </p>
      </div>
    </div>
  );
